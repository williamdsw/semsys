...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> handleObjectNotFoundException(ObjectNotFoundException exception,
		...
		StandardError error = new StandardError(System.currentTimeMillis(), notFound.value(), "Not found!",
				exception.getMessage(), request.getRequestURI());
		return ResponseEntity.status(notFound).body(error);
	}

	@ExceptionHandler(DataIntegrityException.class)
	public ResponseEntity<StandardError> handleDataIntegrityException(DataIntegrityException exception,
			HttpServletRequest request) {
		HttpStatus badRequest = HttpStatus.BAD_REQUEST;
		StandardError error = new StandardError(System.currentTimeMillis(), badRequest.value(), "Data Integrity",
				exception.getMessage(), request.getRequestURI());
	...
	public ResponseEntity<StandardError> handleAuthorizationException(AuthorizationException exception,
		...
		StandardError error = new StandardError(System.currentTimeMillis(), forbidden.value(), "Access Denied",
				exception.getMessage(), request.getRequestURI());
	...
	public ResponseEntity<ValidationError> handleMethodArgumentNotValidException(
		...
		ValidationError error = new ValidationError(System.currentTimeMillis(), unprocessableEntity.value(),
				"Validation Error", exception.getMessage(), request.getRequestURI());
	...
	public ResponseEntity<StandardError> handleFileException(FileException exception, HttpServletRequest request) {
		HttpStatus badRequest = HttpStatus.BAD_REQUEST;
		StandardError error = new StandardError(System.currentTimeMillis(), badRequest.value(), "File Error",
				exception.getMessage(), request.getRequestURI());
		return ResponseEntity.status(badRequest).body(error);
	}

	@ExceptionHandler(AmazonServiceException.class)
	public ResponseEntity<StandardError> handleAmazonServiceException(AmazonServiceException exception,
			HttpServletRequest request) {
	...
	public ResponseEntity<StandardError> handleAmazonClientException(AmazonClientException exception,
			HttpServletRequest request) {
		HttpStatus badRequest = HttpStatus.BAD_REQUEST;
		StandardError error = new StandardError(System.currentTimeMillis(), badRequest.value(), "Amazon Client Error",
				exception.getMessage(), request.getRequestURI());
		return ResponseEntity.status(badRequest).body(error);
	}
	...
	public ResponseEntity<StandardError> handleAmazonS3Client(AmazonS3Exception exception, HttpServletRequest request) {
		HttpStatus badRequest = HttpStatus.BAD_REQUEST;
		StandardError error = new StandardError(System.currentTimeMillis(), badRequest.value(),
				"Amazon S3 Client Error", exception.getMessage(), request.getRequestURI());
		return ResponseEntity.status(badRequest).body(error);
	}

	@ExceptionHandler(MissingServletRequestPartException.class)
	public ResponseEntity<StandardError> handleMissingServletRequestPartException(
			MissingServletRequestPartException exception, HttpServletRequest request) {
		HttpStatus badRequest = HttpStatus.BAD_REQUEST;
		StandardError error = new StandardError(System.currentTimeMillis(), badRequest.value(), "Missing Request Part",
				exception.getMessage(), request.getRequestURI());
		return ResponseEntity.status(badRequest).body(error);
	}
}